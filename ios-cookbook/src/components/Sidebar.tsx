...
import { BookOpen, Heart, Clock, UtensilsCrossed, Plus, Moon, Sun, ChefHat } from 'lucide-react';
...
const navItems: { id: TabType; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { id: 'quick', label: 'Under 20 Mins', icon: Clock },
  ...
  return (
    <aside className="hidden md:flex md:flex-col md:w-64 md:shrink-0 md:h-screen md:sticky md:top-0 bg-white dark:bg-[#1C1C1E] border-r border-black/[0.06] dark:border-white/[0.08]">
      ...
      <div className="px-6 pt-7 pb-5">
        <span className="text-[11px] font-bold uppercase tracking-wider text-[#FF3B30] dark:text-[#FF453A] flex items-center gap-1.5">
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#FF3B30] text-white text-sm font-bold shadow-xs hover:brightness-105 active:scale-[0.98] transition"
        >
          ...
          return (
            ...
            >
              ...
              {item.id === 'favorites' && favoritesCount > 0 && (
                <span className="bg-[#FF3B30] text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
                  {favoritesCount}
                </span>
              )}
            </button>
